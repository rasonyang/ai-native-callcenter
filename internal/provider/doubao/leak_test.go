...
// three owners a session can leave behind and looks for them by name.
...
// Stopping is not instantaneous — a read loop ends when its socket does — so
// the check is given a window to come good in before it fails.
