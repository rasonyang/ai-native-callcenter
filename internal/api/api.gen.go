...
type DID struct {
	...
	// Language Lowercase BCP 47 subtag, at most 8 characters. Sets the greeting and the prompt language, nothing else. It selects neither the provider, which is fixed per deployment, nor the voice, which comes from the flow (global.voice) or, when the flow names none, the provider's default.
...
type ExtensionWrite struct {
	...
	// Password Write-only: required on create, optional on update (empty keeps the current one). Never returned. A manually configured phone registers with it; while the agent is signed in to the panel their handset holds a server-issued SIP session (POST /agent/sip-session) and that credential takes precedence.
