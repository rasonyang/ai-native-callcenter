...
一个语音厂商，启动时选定：中国大陆用 `qwen` 或 `doubao`，其他地区用 `openai`，也可
以用 `gateway` 指向自建的 Realtime 网关。**通话的语言永远不会用来选择厂商。** 其中
三个是同一套协议的不同 profile；`doubao` 走的是另一套协议，有自己的客户端（见
[如何接入一个语音厂商](docs/provider-extension.md)）。
