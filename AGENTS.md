...
2. **AI path**: FreeSWITCH bridges a caller to the `aicc_bot` gateway → `voice.UAS` (custom SIP server on :6060, RTP pool 40000–40999) → `aicall.Session` bridges the leg to a `provider.VoiceSession` (OpenAI = G.711 passthrough, Qwen = 16k/24k PCM16 and the gateway 24k both ways, via `media.Converter`) while `flow.Engine`/`Runtime` steer phases and tools → `aicall.Orchestrator` composes it all, resolves DID→flow (published revision only) and executes transfers by `uuid_transfer`-ing the *caller's* channel (carried in the `X-AICC-Channel-ID` SIP header) to a queue extension.
   ...
   **One provider per deployment, chosen at startup** (phase1-decisions A1): `AICC_PROVIDER` (`qwen` or `doubao` in mainland China, `openai` or `gemini` elsewhere) plus `AICC_PROVIDER_ENDPOINT` / `AICC_PROVIDER_MODEL`, resolved once in `main` via `provider.ProfileFor` and handed to the orchestrator. **A call's `language` never selects a provider** — it sets greeting, prompt language and voice only; there is no language→provider mapping and reintroducing one is a regression.

   **The provider extension point is the wire protocol, not Go.** `internal/provider` is an OpenAI-Realtime client × `Profile`. A new *engine on that protocol* is a new *profile* — name, endpoint, model, dialect — and a new `AICC_PROVIDER` value, never a second client. That is how the phase-2 **OpenAI Realtime Gateway** attaches (a *separate service* composing ASR + LLM + TTS behind the same events, impersonating no vendor — A6). That profile exists: `AICC_PROVIDER=gateway` (`GatewayProfile`, 24 kHz linear both ways, GA dialect), verified on live calls 2026-09-02. Cascade never enters this repo: no ASR/LLM/TTS types, interfaces, adapters, stubs or TODOs. **Only a new *protocol* earns another client** — the same sentence `internal/transcribe` has held two clients under since it was written. Two have, so there are three clients: the Realtime one × its profiles, plus `AICC_PROVIDER=doubao` (`internal/provider/doubao`, ByteDance full-duplex, `docs/design/doubao-findings.md`), because the divergence is lifecycle and not field names — session bootstrap with no way to ask for a turn, no speech events, role-`tool` results, a close handshake, a paced-and-muted uplink — and `AICC_PROVIDER=gemini` (`internal/provider/gemini`, Google Live bidirectional, `docs/design/gemini-findings.md`), for the same kind of reason: a session configured once and never reconfigurable, two separate turn endings (model stopped ≠ caller heard), no cancel primitive at all, errors only as a close code, and a downlink that arrives several times faster than the caller can hear it. Another client lives in its own sub-package behind the same provider-neutral `VoiceSession`, shares transport/audio plumbing only where ownership and runtime semantics are identical (`wsconn`, `Watchdog`, `MergeHint`, `SayExactly`, `internal/provider/pacer`) and shares no protocol event, decoder or dispatch. `VoiceSession` is the seam between the call actor and whichever client answers (plus its test fake); do not "generalise" it — another client goes *under* it, saying the same things, and `provider.Event.FailureCause` is how one names a fatal ending the CDR should keep (`PROVIDER_SESSION_EXPIRED` today). The name→client choice lives in `cmd/aicc/wiring.go`, because a sub-package cannot be constructed from inside its parent.
...
Flow DSL v2 (`internal/flow`): the model owns the conversation, the flow owns the phase. The flow also owns the bot's **voice** — `global.voice`, published and versioned with the persona (A7), never an env var; empty falls back to the provider profile's. Voice names are provider-specific and a deployment runs one provider, so a flow names a voice its own deployment offers. Phases carry instructions + tool allowlists; transitions fire on tool results with structured-operator conditions (rules can test `result.ok`); a fired transition overrides the tool's hint with the new phase's instruction. A phase may also carry `announce`, the one field that is **words rather than a subject**: a bilingual line said as written on entering the phase, rendered with `{slots.x}` like the instruction, for a closing line nobody should paraphrase — and for the engines that take no text cue at all. It reaches the provider as `SessionConfig.OpeningText` (entry phase, asked for while the session starts) or `VoiceSession.SpeakText` (every other phase), both of which **pre-empt and never queue** — a second line replaces the first — and are **best effort on the Realtime client and on gemini**, both of which can only direct a model to repeat a sentence (`provider.SayExactly` is that demand, shared; on qwen, `NeedsDirectedLineInConversation`, a mid-call line that ends the call — `SpeakText(text, isClosing)`, set for a terminal phase — also enters the conversation as a caller message ahead of the request, because the override alone lost to the history there — W-Q1; a line the call goes on from keeps the override alone). One exception: a tool result that moves the call into a *terminal* phase with a line, on a profile with `PutsTerminalAnnounceInToolResult` (openai/qwen/gateway), carries the line as its hint (`SayExactly`) — the phase re-pinned and the ending armed before the tool is answered — and the turn the result produces is the line; no `SpeakText` follows (qwen-findings W-Q1, A5c). Doubao (exact text) and gemini (W-G6, an unanswered tool result) keep `SpeakText`. A turn stopped to make room reports `INTERRUPTED{by: SYSTEM}`, not SPEECH. `provider.Profile.RequiresTerminalAnnounce` (false for openai/qwen/gateway/gemini, true for doubao, whose engine takes no text cue at all) makes `flow.RequireTerminalAnnounce` refuse to *publish* a flow whose terminal phases have no line — publish, not load, because the rule belongs to the deployment and not to the dialect. Built-ins (`transfer_to_agent`, `take_message`, `hangup`) may *refuse*, and a refusal is a conversation ("queue closed → offer to take a message"), not an error. Everything checkable is validated at load, not mid-call. A caller's decline is otherwise invisible to the engine — only `TOOL_RESULT` and `NO_INPUT` are events — so `global.closingTarget` (a terminal node) lets it end a call on its own: a NO_INPUT default (`noInput.count GTE 3`, consecutive — reset by caller words, a keypress or a tool result, never by an empty transcript) where no authored rule can fire on NO_INPUT (never reached on doubao, whose refused dead-air cue produces no further NO_INPUT), and `global.maxTurnsWithoutTool`, which counts completed bot replies to caller speech that made no tool call (a tool call or a phase change resets it) and, once they *exceed* it, moves the call when the caller has heard the reply that crossed it. Both go through the same terminal-arrival path (arm-before-speak; the goodbye cue where the closing phase has no line) as any other ending, as a backstop to closing phases that tell the model to hang up on the first decline (issue #9).

## Invariants that came from live debugging — do not regress

- **Turn end ≠ playback end.** `TURN_DONE` means the model stopped producing; `PLAYBACK_DONE` means the caller heard it. Transfers/hangups execute only after the latter (gated on turn identity), with a 10s cap; a turn cut short never counts as the closing line having been said. Barge-in's boundary is the last frame *heard*: speech over a finished turn's still-playing tail is an interruption (local flush only — never send the provider a cancel once generation ended; Qwen errors on it).
