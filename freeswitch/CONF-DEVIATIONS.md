...
| `autoload_configs/acl.conf.xml` | Added an `aicc_esl` list: loopback plus the three RFC1918 ranges. Added an `aicc_sip_local` list: loopback only. Vanilla's `lan` and `domains` lists untouched. | Unset, `mod_event_socket` applies `loopback.auto` and rejects the application, which lives at another address on the compose network; `localnet.auto` alone would shut `fs_cli` out of the switch's own container. No site's subnet is in the file — the port is not published, so the list is not the access control, and a real address would only document the office it was written in. `aicc_sip_local` is the internal profile's local network; see `sip_profiles/internal.xml`. |
...
| `autoload_configs/callcenter.conf.xml` | Stock `support@default` queue removed; `<queues>` empty, commented agent/tier examples renamed to bare `1000` / `support`. Vanilla's settings block and WARNING comments kept. | aicc_xml.lua renders callcenter.conf from the database, but this file is the fallback when that handler declines on an unreachable database — vanilla's queue would then be installed under a name carrying a domain, which mod_callcenter persists. A queue name carries no domain at any layer (C1); an empty block installs nothing. The agent `contact` stays domain-qualified because an endpoint address genuinely is. |
| `sip_profiles/internal.xml` | `context` `public` → `aicc`. `local-network-acl` `localnet.auto` → `aicc_sip_local`; `aggressive-nat-detection=true` uncommented in place. | The single most load-bearing change. A phone's INVITE takes the profile's context, and in `public` the stock `public_extensions` rule transfers `10xx` into the stock `default` dialplan before any aicc rule is reached — `dialplan/aicc.xml` would be installed, valid, visible to `xml_locate`, and never executed. Design 01 §7 D6a records the three things tried first that did not work. The local network: a phone reaching the container through Docker's port forwarding arrives from the compose network's gateway address, which is private and inside `localnet.auto`. Sofia then took the phone to be on its own link, skipped NAT handling and answered with the container's address (`10.130.0.3`) in the SDP instead of `ext-rtp-ip`: the switch's audio reached the phone, the phone's audio went to an address it cannot reach, and a bot call ended on the media watchdog after 5 s. With only loopback local, every phone is answered with `ext-rtp-ip` (`FS_EXTERNAL_IP`) and RTP auto-adjust follows the address the media actually arrives from; `disable_rtp_auto_adjust` must stay unset. The external profile, which carries the `aicc_bot` gateway, keeps `localnet.auto`, so the bot leg still advertises the container's address. Verified only on Docker Desktop on an Intel Mac; not re-verified on a Linux host. |
