  ...
  return (
    <>
      ...
      <DataTable>
        ...
        <TBody>
          {isPending && <TableMessage colSpan={4}>{t('common.loading')}</TableMessage>}
          {isError && <TableMessage colSpan={4}>{describeError(error, t)}</TableMessage>}
          {!isPending && rows.length === 0 && (
            <TableMessage colSpan={4}>{t('admin.noExtensions')}</TableMessage>
          )}
          {rows.map((row) => (
            <Tr key={row.id}>
              ...
              <Td className="text-xs text-muted-foreground">
                {/* Who it belongs to. An unbound phone is a phone nobody has
                    been given, which is the only thing "no owner" can mean now
                    that a phone arrives with its account. */}
                {row.agentId ? t('admin.bound') : t('admin.unbound')}
              </Td>
              <Td className="text-xs text-muted-foreground">
