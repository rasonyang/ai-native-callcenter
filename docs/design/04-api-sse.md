...
**Machine access (2026-08-26)**: a system with no browser authenticates with `X-AICC-Api-Key`, matched against `AICC_API_KEY` and accepted on `POST /calls` alone — mounted apart from the session-only group so the surface is the route list, not a middleware flag. Unset leaves the path closed, which is what an unset variable means to the config loader. A presented key is answered on its own terms and never falls through to the session rules; no CSRF header is asked of it, that header existing to protect a credential the browser sends automatically. The key authenticates the deployment's integrator, not a person: it acts as SUPERVISOR (not ADMIN — configuring the platform stays a person's job), and its audit rows carry a null actor and name `api-key` in the detail rather than inventing a user id. One shared secret, no per-integrator identity, rotated by restarting.
