...
What is lost is only wording: a caller on hold music reads `Dialing`, which is an odd thing to call them. That is a naming question about an existing state, not an argument for a new one — if it ever needs answering, the answer is to rename, not to add. `EventQueued` and `EventDestinationBusy` leave with it; a busy destination is a release carrying that cause, which is how every other ending is already reported.
...
**Amendment — the phone is issued its credentials, not configured with them (2026-09-12).** An agent never types a server, an account or a password into the extension. On login the SPA calls `POST /agent/sip-session`; the platform generates a password for that agent's extension, stores `a1_hash = md5(account:sipDomain:password)`, discards the plaintext and answers with `{sipDomain, wssUrl, account, a1Hash, expiresAt}`, which the SPA hands to the extension over same-origin `postMessage` ([../web-sip-phone-protocol.md](../web-sip-phone-protocol.md)). `expiresAt` is the web session's expiry, so the phone is signed in for as long as the person is. One active session per agent: issuing a new one replaces the previous and flushes the registration it was holding (`sofia profile <profile> flush_inbound_reg <account>@<domain>`), which is also what `DELETE /agent/sip-session` and signing out do. Four consequences:

- **The realm is now a value we choose, not one we discover.** `AICC_SIP_DOMAIN` (defaulting to `AICC_SWITCH_DOMAIN`) is both the digest realm and the domain of the address of record, and it works because the internal profile challenges with `challenge-realm=auto_from`: the realm the phone is asked for is exactly the domain it put in its `From` header, which is the domain it was issued. `AICC_SIP_WSS_URL` (defaulting to `ws://<AICC_SIP_DOMAIN>:5066/`) is the profile's `ws-binding` published to the phone. The paragraph above still describes the realm *observed* in the Caddy dev topology; the lookup rule it produced — key on the user id, never on domain equality — is unchanged and is what lets the two coexist.
- **The static extension password becomes the fallback credential** (*amended 2026-09-16*; as first written it no longer authenticated a registration at all). The directory handler serves one credential per user and the session's comes first: `<param name="a1-hash">` from the agent's active SIP session, and `<param name="password">` from `extensions.password` when there is no live session. Never both — FreeSWITCH verifies a single credential, and which one it used should not be the switch's choice. The refusal survives, narrowed: a `sip_auth` lookup that produced no credential at all still returns nothing. What this buys is the phone the platform did not provision — a handset configured by hand registers again, which is also the static-credential path phase 2's hardware phones need, so it no longer has to be re-opened later.
- **`allow-empty-password=false` is emitted on every directory user, always.** `sofia_reg.c` authenticates a user that carries neither `password` nor `a1-hash` unless that parameter says otherwise — so a handler that answered a lookup with no credential in it would be handing out registrations, not refusing them.
- **Device readiness is derived from the switch, not from the browser.** `sofia::register` / `sofia::unregister` / `sofia::expire` (already subscribed) set `isDeviceRegistered` and `deviceAccount`, and every ESL connect reconciles them from `sofia status profile <profile> reg`, because a registration that arrived while the link was down is otherwise invisible. `POST /agent/ready` answers 409 `DEVICE_NOT_REGISTERED` without one, and losing the registration while READY moves the agent to NOT_READY with reason `DEVICE_LOST` — which the platform withdraws again when the phone re-registers, returning that agent to READY, because `DEVICE_LOST` is the only reason the platform set on its own. The fields ride `GET /agent/presence`, `GET /auth/me` and the `AGENT_*` SSE payload.
...
**Amended 2026-09-12, again 2026-09-16**: the `directory` binding serves exactly one credential per user, with `allow-empty-password=false` beside it always. It is the a1-hash of the agent's *active* SIP session where there is one, and `extensions.password` where there is not, so a phone the platform provisioned and a phone somebody configured by hand can both register. It refuses a `sip_auth` lookup only when neither exists — see §3's amendment for why `allow-empty-password` is not optional.
