...
- **The static extension password no longer authenticates a registration.** The directory handler serves `<param name="a1-hash">` from an agent's active SIP session and refuses a `sip_auth` lookup when there is none. `extensions.password` and `GET /extensions/{extensionId}/password` are kept for API compatibility and nothing else. This needs revisiting when phase 2 adds hardware phones, which cannot be provisioned this way and will need a static-credential path of their own.
...
**Amended 2026-09-12**: the `directory` binding no longer serves a password. For a `sip_auth` lookup it answers with the a1-hash of the agent's *active* SIP session and `allow-empty-password=false`, and refuses the lookup when there is no active session — see §3's amendment for why both halves are needed.
