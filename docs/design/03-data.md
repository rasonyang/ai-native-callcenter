...
- **The cast grew by two.** Administration and supervision are role-gated, so a demo seeded with agents only hid most of the product: `admin` (ADMIN) and `sam` (SUPERVISOR) join `amy`/`ben`/`cara`. Extensions carry the documented `demo1234` as their SIP password rather than a random one — an unregisterable softphone is not a demo either. There is no forced-change flag: the schema has no such column, and the demo dataset is opt-in.
