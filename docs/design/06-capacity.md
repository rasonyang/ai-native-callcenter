...
**Metrics added after M5**, in the same file, outside §6's list: `aicc_provider_sessions_started_total{provider}` (session-creation rate, the quota vendors limit), `aicc_provider_sessions_expired_total{provider}` (sessions the provider ended at its own lifetime cap; a subset of `aicc_provider_ws_errors_total`), `aicc_bot_interruptions_total{reason}` (`SPEECH` or `DTMF`; speech inside the barge-in guard is not counted), `aicc_webhook_deliveries_total{subscriptionId,outcome}` (`DELIVERED` or `FAILED`), and `aicc_transcribe_frames_sent_total` / `aicc_transcribe_frames_dropped_total{provider,speaker}` (audio handed to, or discarded before, a recognition session). `aicc_turn_latency_ms` is published by `obs.RecordTurnLatency`, fed from `internal/aicall/metrics.go`.
