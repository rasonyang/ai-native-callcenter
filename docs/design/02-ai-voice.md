...
Turn-detection config mapping: `VAD` → `server_vad` with `SilenceMs` (**the default for both languages**: Qwen's own default 800 ms is lowered to 500 ms — M0-verified as accepted and echoed; OpenAI's GA default is already 500 ms). `Semantic` → Qwen `smart_turn` / OpenAI `semantic_vad`, supported by the provider layer (`TurnModeSemantic`, `Profile.SemanticTurnType`) but **not selectable today**: no flow field chooses it, and every call uses `provider.DefaultTurnDetection()` (`server_vad`, 500 ms) in `sessionConfigFor`. M0 found `smart_turn` forces `silence_duration_ms = 2000` and ignores attempts to lower it, i.e. ≈+1.5 s of turn latency — worth it only where backchannel immunity (an "mm-hm" not interrupting) outweighs snappiness. Both modes are frozen after the first audio frame on Qwen, so any future choice has to be made before `Start`. DTMF always interrupts immediately (policy from golang-bot field data).

## 6. Flow engine (DSL v1) over realtime sessions

Spec = **DSL v2**: ui-test's v1 re-keyed to lowerCamelCase per 07 §7 (`specVersion:"v2"`): `global{persona, rules, voice, fallback, closingTarget, maxTurns, maxTurnsWithoutTool, tools, transitions}`, `nodes{instruction, announce, tools, transitions, isTerminal}`, `tools{description, params(JSON-schema), http{path, body template, success predicate, result slots}}` — all user-facing strings bilingual `{en,zh}`; the session uses the entry point's language. The five v1 reference flows are converted by a one-shot script; v1 files stay reference-only.
...
`transfer_to_agent` sequencing: tool call → engine replies with a result whose hint says "tell the caller you're connecting them" → on that response's **`PLAYBACK_DONE`** (or the 10 s cap, `actionGraceCap`) → execute the transfer. ⚠ **Amended in M3**: waiting on `ResponseDone` is wrong — the model finishing generation and the caller finishing hearing are separated by everything still in the RTP send queue (up to seconds), so a transfer on `ResponseDone` cuts the bridge line off mid-word. `aicall` therefore reports the two separately: `TURN_DONE` when the model stops producing, `PLAYBACK_DONE` when the queue has drained and one further frame interval has passed. Anything that must not interrupt the bot — transfer, hangup, goodbye — waits for the second. (golang-bot does the same thing, waiting on `sink.Pending()` before declaring a prompt finished.)

**A transfer can be refused, and the bot handles it in conversation.** Before executing, the engine checks that the target queue exists (or a fallback queue does) and is enabled; otherwise the tool returns `ok:0` with a hint naming the reason (`QUEUE_UNKNOWN`, `QUEUE_CLOSED` — the latter meaning the queue is disabled) and a refusal hint, so the model says something like "we're closed right now — I can take a message" and continues with `take_message`. Business-hours and staffing checks (`NO_AGENTS_STAFFED`) are not implemented. The design places business hours on the queue and not on the DID: an AI-native call center has no separate out-of-hours branch in the dialplan, because the bot has already answered and can simply explain (owner decision 2026-08-13).
