...
| `internal/provider/doubao` | the second client, for ByteDance's full-duplex dialogue protocol (`AICC_PROVIDER=doubao`) — a different protocol, not a dialect (A6 as amended, `doubao-findings.md`). Behind the same `VoiceSession`, producing no event the first client does not | provider, wsconn, pacer |
| `internal/provider/gemini` | the third client, for Google's Live bidirectional protocol (`AICC_PROVIDER=gemini`) — a third protocol, on the same rule and for its own reasons (A6 as amended, `gemini-findings.md`). Behind the same `VoiceSession`, and the one client that also runs an energy detector of its own, narrowly (§`gemini-findings.md` 2.7) | provider, wsconn, pacer |
| `internal/provider/wsconn` | what every client's socket does identically: dial with a deadline, single-writer send, ping/pong keepalive, close. Transport only — no protocol event crosses it | — |
| `internal/provider/pacer` | the uplink cadence: a bounded queue the call path never blocks on, and one goroutine deciding when a frame goes out and which do not. Carries frames and names no protocol — the client supplies the encoding and the write | — |
