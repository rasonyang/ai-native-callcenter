...
| # | Decision |
|---|---|
| A1 | en → OpenAI Realtime (`gpt-realtime-2.1` default); zh → Qwen `qwen-audio-3.0-realtime-plus` default (`-flash` selectable). Provider + model configurable per flow/entry point; selection is a routing-layer rule (never hardcoded). |
| A2 | **Language determined by per-DID/entry-point config**; no mid-call provider switching in MVP (Qwen turn_detection frozen after first audio; switch = re-bridge, roadmap). |
