...
| G4 | **Zero-config agent phone (owner directive 2026-09-12).** An agent configures nothing: signing in to the SPA issues that browser a SIP session and hands it to the extension. Decisions taken with it, each with its reason: **(a)** the operation is `POST` / `DELETE /agent/sip-session`, not `/me/sip-sessions` — a self resource is `/agent/*` everywhere else in this contract, and one active session per agent makes the collection plural for no one. **(b)** The readiness fields are `isDeviceRegistered` and `deviceAccount`, per [07-naming](design/07-naming.md) `is_` booleans; `deviceAccount` is nullable because "registered as nothing" is a state and an empty string is not. **(c)** `AICC_SIP_DOMAIN` defaults to `AICC_SWITCH_DOMAIN`: a deployment that has not separated the switch's domain from the phones' realm has one domain, and should have to say nothing. **(d)** The static extension password is retired **for authentication** and kept on the API for compatibility (`extensions.password`, `GET /extensions/{extensionId}/password`). This conflicts with the phase-2 hardware-phone roadmap — a desk phone cannot be provisioned by a browser — which will need a static-credential path of its own, deliberately re-opened then rather than left half-alive now. **(e)** FreeSWITCH `sofia_reg.c` authenticates a directory user carrying neither `password` nor `a1-hash` unless `allow-empty-password=false` is present, so the Lua handler emits that parameter on every user and refuses a `sip_auth` lookup that has no active session behind it; without both halves, retiring the password would have opened registration rather than closed it. **(f)** The `ExtensionSecret` "D4" rationale — that an a1-hash is unusable here because the realm follows the host address and a moved address cannot be recomputed — is **superseded**: a session is minted at every login, so a realm change costs one sign-in and nothing else. The clear-text column survives that reasoning only as compatibility. |
