...
| A7 | **Voice is bot configuration, not deployment configuration**: `global.voice` in the flow spec, stored in the database with the rest of the bot and published with it — never an env var. It is not per language: both OpenAI Realtime and Qwen-Audio Realtime offer voices that carry Chinese and English well, so a bilingual bot keeps one voice. Voice names belong to the provider that answers, and a deployment runs one (A1), so a flow names a voice its own deployment offers; empty falls back to the profile's default. (Owner directive, 2026-08-16.) |

## Agent side & auth

| # | Decision |
|---|---|
| G1 | **web-sip-phone integrated as-is; coordination is backend-driven**: ESL observes/controls (auto-answer `Call-Info: answer-after`, `uuid_phone_event talk/hold`), SSE feeds the SPA softphone bar + incoming-call popup. The extension is never messaged programmatically; SPA origin must be in its Allow Sites. **Amended 2026-09-12 (G4):** the SPA does message the extension, for credentials and status only — call control is unchanged and stays backend-driven, and "allow this site" is now a step the product walks a person through rather than an instruction in a document. |
| G2 | **Dev topology (documented reference setup)**: external Caddy (`~/workspaces/github/proxy/Caddyfile`, outside this repo): `app.aicc.test`→Vite :5173, `api.aicc.test`→Go :8080, `ws.aicc.test`→FS wss :7443 (TLS-to-TLS with skip_verify; sofia requires Via transport = socket transport). The repo itself stays HTTP-only. **Amended 2026-09-12 (G4):** the phone is told where to connect (`AICC_SIP_WSS_URL`) and which realm to authenticate against (`AICC_SIP_DOMAIN`) instead of deriving both from whatever hostname it was configured with, so this topology is one deployment's values rather than an assumption in the client. |
...
| G4 | **Zero-config agent phone (owner directive 2026-09-12).** An agent configures nothing: signing in to the SPA issues that browser a SIP session and hands it to the extension. Decisions taken with it, each with its reason: **(a)** the operation is `POST` / `DELETE /agent/sip-session`, not `/me/sip-sessions` — a self resource is `/agent/*` everywhere else in this contract, and one active session per agent makes the collection plural for no one. **(b)** The readiness fields are `isDeviceRegistered` and `deviceAccount`, per [07-naming](design/07-naming.md) `is_` booleans; `deviceAccount` is nullable because "registered as nothing" is a state and an empty string is not. **(c)** `AICC_SIP_DOMAIN` defaults to `AICC_SWITCH_DOMAIN`: a deployment that has not separated the switch's domain from the phones' realm has one domain, and should have to say nothing. **(d)** The static extension password is retired **as the agent's credential** and kept as the fallback one (`extensions.password`, `GET /extensions/{extensionId}/password`). *Amended by the owner directive of 2026-09-16*: as first decided it was retired for authentication outright, which left a hand-configured handset with no way to register at all and turned "browser-phone first" into "browser-phone only". The directory now offers one credential per extension and the session's comes first — the a1-hash while the agent is signed in, the static password when nobody is. Zero-config is unchanged and is still the path an agent takes; manual configuration is a working fallback, and it is also the static-credential path the phase-2 hardware-phone roadmap needs, so that conflict is settled rather than deferred. **(e)** FreeSWITCH `sofia_reg.c` authenticates a directory user carrying neither `password` nor `a1-hash` unless `allow-empty-password=false` is present, so the Lua handler emits that parameter on every user and refuses a `sip_auth` lookup that produced no credential at all; without that parameter an extension with an empty password and no session would authenticate anything, which is what the password's retirement would otherwise have opened. **(f)** The `ExtensionSecret` "D4" rationale — that an a1-hash is unusable here because the realm follows the host address and a moved address cannot be recomputed — is **superseded**: a session is minted at every login, so a realm change costs one sign-in and nothing else. What the clear-text column survives that reasoning for is (d): it is the credential of the phone no login provisions. |
