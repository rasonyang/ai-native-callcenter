...
**任务已完成。①–⑦ 全部落地，四道人工门全过。** 这份文件此后只作为记录：
要理解「为什么授权是这个形状」，读它和 `docs/auth/RESULTS.md`（证据账本，含每一步的失败形态与反证）。

若要在这套模型上继续改动，先记住四条：

- **契约是执行的，不是描述的。** 每个 operation 的 `security` 由 `scripts/gen-opsecurity.mjs` 生成成
  `api.OperationSecurityByRoute`，`enforceContract` 在生成 wrapper 内部读它。加路由不改契约 = 那条路由没有授权，
  会以 500 失败关闭，并被 `internal/httpapi/contract_gate_test.go` 当场拦下。
- **handler 内的授权判定要登记进 `docs/auth/scopemap.py` 的 `HANDLER_CHECKS`。** 脚本读不到路由表以外的东西，
  `createAICall` 就是这样漏掉过一次，差点把外呼机器人发给每个坐席。
- **勾选的粒度不能大于验证的粒度。** ②a 曾因为跑完前半就整条勾掉，谎报了一个没写的生成器。
- **通过的断言在证明它抓得住之前不算数。** ⑦ 的三条、⑤ 的迁移 fixture、吊销后不可编辑那条，都注入过缺陷验证。

跑起来：`docker compose -f deploy/dev/docker-compose.yml up -d`，
`AICC_TEST_DATABASE_URL='postgres://aicc:aicc@127.0.0.1:5432/aicc?sslmode=disable'`，
`go test -race ./...` / `make api-check` / `python3 docs/auth/scopemap.py`。
...
- [x] **REVOKED 终态的首次真实执行（人工门）—— owner 确认，2026-08-31**。
      在真服务器上：吊销 200 → 同一把 Key 401 → `last_used_at` 一个字没动（被拒的请求不算一次使用）
      → 重复吊销 409 → 吊销后 PATCH 409。逐条形态见 `docs/auth/RESULTS.md`。
