...
### V1 — 未知的 `/api/v1/*` 路径返回 `200 text/html` ★最严重
...
`[INFERENCE]` 修法：在 `/api/v1` 子路由上单独挂 JSON 版 `NotFound`（子树自己有了 handler，chi 的传播就不会覆盖它）。三行。

### V2 — 方法不匹配返回 `405` 空 body，不带 `Content-Type`，不是错误信封
...
`[INFERENCE]` 同 V1，在 `/api/v1` 子路由上挂 `MethodNotAllowed`。
...
| # | 处置 | 落在哪 |
|---|---|---|
| V1 V2 | `/api/v1` 子路由挂 JSON 版 `NotFound` / `MethodNotAllowed` | `[ASSUMPTION]` 独立小提交，不进认证任务的 7 个提交；契约无需改（404/405 已在契约里） |
