...
| audit_logs(00005:127) | sql/ledger.sql InsertAuditLog(auditTrail 中间件 httpapi/audit.go) | **ListAuditLogs/CountAuditLogs ← httpapi ListAuditLogs(`GET /audit-logs`,ADMIN)** | **`/admin/audit`** | NOT FOUND | S? | [FACT] | **2026-08-24 W4 已闭**:检索按 action 前缀(存的就是 `METHOD /route/template`),操作者名读时 LEFT JOIN 解析、账号删了仍留 id。同批修 **C58**:该表曾存明文分机 SIP 密码(4 行),现按字段名递归脱敏 |
