...
> **W8 已撤销**(2026-08-24 owner 直裁:删表 + 只读状态,见条目);**W10 已完成**(2026-08-25;实质早由 C47 达成,见条目);**W7 已完成**(2026-08-25;十个零生产者 SSE 类型清零 —— 补齐五个、删掉两个永不到来的、打断改走指标);
...
> 阶段 0–6 已完成。阶段 7:**W1–W7、W10、W11 已完成,W8 / W9 已撤销 —— W 系列全部收官**(此行原写"W1–W9 未开工",是 v1 发布时的笔误,2026-08-24 更正);
> **C 系列:C1–C60 全部有结论,无待办**(2026-08-25 收官)。C10 已决 defer 第二期;C42 四步做完并移出;C32/C14 不复现。~~真开 1 项(C57)~~ —— **C57 已于 2026-08-25 查清结案**(CDR 时长字段**不构成划分**,那个恒等式从未被设计过;修的是文档口径,见条目)。C37 于 2026-08-24 查明成因(web-sip-phone 的 RESET 出路只有一个后台定时器)并由该仓修复,aicc 侧零改动;~~**遗留一个已知缺口:486 不计入 `max_no_answer`**~~ **【2026-08-25 查清:486 不计数是 mod_callcenter 的设计
(忙/拒接/没接是三件事,只有第三件算"没接"),交换机侧的兜底一直在(`busy_delay_time=60`);真正的缺陷在我们这边 —— 坐席拒接被当成没接,已修。见 C59。
并由此做掉 A1/A2:坐席在通话中,队列不再派单给他**(见 C59 与其后两条);**C34 已于 2026-08-24 现场闭合**(实为四个接口在说谎,见条目);**C1 两半均已修**(后半 2026-08-24,但重跑 VC-S3-02 前仍须重造场景,否则假通过);**C24 已于 2026-08-24 现场闭合**(修它的是 08-22 的 aicc context 三连,见条目);**C32 已不再复现**(原因未证明,守卫为 VC-S14-04);**C14 在 qwen 路径上不复现**(配置变了,不是同配置下消失;openai 路径未测)。
> 上一行的 "4 PASS / 1 FAIL / 23 TODO" 是 v1 发布时的**输入基线**,作为历史保留不改。
>
> **【2026-08-25 收尾盘点,owner 定】** 账本 **41 case / 41 PASS / 0 FAIL / 0 TODO**
> (39 → 41 是 2026-08-25 新增的 VC-S15-01 / VC-S15-02);W 系列全部收官;
> **C1–C62 全部有结论**(C61 当日查清并修复;C62 是 C55 §4 的重复立案,当日以删除字段结案)。
> **本文件余下两件不再挂在这里,另起任务处理**:
> - **L2–L5 基准活动** —— 本来就不在本文件跟踪(计划在 `docs/load-tests.md`),
>   owner 2026-08-16 起长期推迟:需要非开发笔记本的宿主、非开发用的交换机、`sipp` 与 provider 额度。
>   **⚠ 在它真正跑过之前,任何地方都不得出现容量或延迟数字**(owner 指令,见 CLAUDE.md)——
>   本行不构成"已测",只表示**不在本文件的待办里**。
> - **T6.13 排期缺口** —— 已关闭,理由见该节。
>
...
### ~~⚠ 计划缺口(2026-08-22 owner 提问暴露)—— 新并入的 11 条没有执行阶段~~ **【已关闭 2026-08-25】**

**这一节已经作废,原文保留在下面备查。** 缺口自行消解了:那 11 条于 2026-08-23
全部并入 `ledger.yaml` 并执行完毕,账本现为 **41 case / 41 PASS / 0 TODO**,
补一个 T6.13 执行阶段已无对象。**不要再照下文去排期** —— 读到这一节的人会以为
还有 11 条没跑。原文:
...
- ~~**W9 账号清理**(§补充 S4,含 seed 名单同步)~~ **【已撤销 2026-08-25,owner 直裁:「W9 账号保留」】**
  理由是一条**路线事实**,不是对这批账号的偏好:**第一期生产环境只支持 web-sip-phone Chrome 扩展,
  后期仍要支持原生 SIP 话机、包括硬话机。** 那么本机这部原生软电话就不是遗留物料,
  而是**下一期要支持的那一类终端在今天的唯一样本** —— C37 的定性完全靠"浏览器话机 7 : 原生软电话 0"
  这个对照得出,C24/C52/C55 的现场验证也用两部不同实现的话机互为对照。
  一期还没做的支持,不该在一期就先把验证它的能力删掉。
  **连带作废**:seed 名单不再删 amy/ben(`demoPeople` 不动);VC-S11-02 也不必改写成用 `agent` 抢占,
  amy 还在,原样可重跑;F11(非 seed 账号的口令与归属)维持"记档、不阻塞"。
  下面这段是原规格,保留作历史:live DB 只保留 **wei、agent、supervisor、admin**,其余
...
- **C57 已查清并结案(2026-08-25)** —— **那个恒等式从来没有被设计过,减法从第一天起就不成立。**
  **实现侧**:`queue_wait_sec` 量的是 `JoinedAt → BridgedAt`(未接则到 `LeftAt`);
  `ring_sec` 量的是坐席腿 `CreatedAt → 首次 bridge`(未接则 `ringSpan`:首腿起、末腿释放止)。
  **mod_callcenter 是在主叫等待期间去派单的** —— 所以振铃不是排队之后的一段,
  **是发生在排队窗口里面的一段**。它们不是"同一段时间的两种说法"(我上次的措辞不准确),
  是**一段与它的子段**,两者在挂断那一刻同时结束,于是 207/207 这种数字看起来像相等。
  **设计侧**:设计 03 只说这几个字段 *"come from party-established and callcenter event timestamps"* ——
  **时间戳测量,不是时段划分**;全套设计集里**没有一处写过它们相加应等于 `total_sec`**。
  更直接的是:**契约里 `queueWaitSec` 的描述早就写着**
  *"Ringing happens inside this window, so ringSec is a sub-interval of it rather than an addition to it."*
  —— 答案一直在契约里,是我做那次筛查时没读。
  **最好笑也最说明问题的一处**:`TestAssembleAnsweredCallSplitsTheDurations` ——
  **名字断言了划分,而它自己的数字否定了划分**:bot 30 + wait 20 + ring 5 + talk 50 = **105**,
  通话总长 **100**。这条重叠一直摆在最经典的那条用例里,没人看它一眼。
  已改名为 `TestTheDurationsMeasureTheCallRatherThanPartitionIt`,
  并**把"四段之和 105 > 总长 100"写成断言** —— 谁要让它们加得起来,先得改这条用例并在此说明理由。
  **全库复核(6389 行,不再是当初那 320 行)**:
  ```
  四段之和 > total_sec                      58 行
  去掉 ring 之后仍然超出                     12 行 —— 全部落在 2026-08-17…21,08-21 之后再无
  2026-08-22 以后的超出                     每一行的超出量 == ring_sec,分毫不差
  ```
  **也就是说:08-22 之后,`ring_sec` 是唯一的重复计数,而它是有文档的那一个。**
  那 12 行(`queue_wait` 把 `talk` 一起吞了,如 01a01eae:total 186 / wait 164 / talk 156)
  **末次出现 2026-08-21**,而 08-22 之后 14 通"入队且被接起"的电话一例都没有 ——
  应是被 08-22…24 那批修复(C36 收养 / 队列桥接跟踪)顺带治好的;**未独立复现,故不改判、只记录**。
  **结论是文档,不是代码**(条目自己写的:"如果从来没人这样设计过,那么这个减法从第一天起就不成立")。
  已把口径写进契约:`totalSec` 的描述明说其余四个**不构成对它的划分**、相减得出的数没有意义、
  相加可以超过它;`ringSec` 补上"未入队的通话没有队列窗口可供它落在里面"。
  **哪些是真的**:`botSec` → `queueWaitSec` → `talkSec` 是主叫依次经过的三段,**它们相加确实装得下**
  (用例已钉);`ring_sec` 才是那个不是"一段"的东西。
  **另立两条小账,不并入本条**:
  ①~~**74 行 `INBOUND` 而 `bot_sec=0`**(2026-08-17…24,至今仍在发生)——每通来电都先见 bot,故存疑~~
  **【已查清 2026-08-25;绝大多数不是缺陷,但查出一处可修的真缺口 —— 见 C58】**
  ②~~**5 行 `ring_sec > total_sec`**(末次 2026-08-22)——应答腿的 `CreatedAt` 早于通话记录本身,
  机制未查明~~ **【已查清并结案 2026-08-25 —— 见 C60】**。两条都曾**只记不判**,与 C57 的口径问题无关。

- **C60(2026-08-25,由 C57 的小账②查出)** **【已查清;5 行是旧疾,但挖出一条今天还在发生的,已修】**
  **一、那 5 行 `ring_sec > total_sec` 是修复前的产物,已被治好。**
  全部落在 2026-08-20…22,形状一模一样(以 `01a01f41` 为例):
  ```
  INTERNAL ANSWERED  total=3  ring=12  talk=3   legs 只有 1 条
  started  13:00:01.974
  answered 13:00:01.907    ← 比 started 早 67 毫秒
  ```
  **一通内部呼叫被拆成了两行 CDR**:1008 于 12:59:20 拨 1007,主叫腿自成一行
  (`01a01f40`,只有一条 `DIALING` 腿);被叫接起时又出现第二行(只有一条 `AGENT` 腿)。
  `ring_sec` 量的是那条早 12 秒创建的腿,`total_sec` 从后一行的诞生算起 —— **所以 ring 比 total 长**。
  正是 `aicc.xml` 注释里记着的 C24 形状(*"each successor leg becomes its own CDR row"*)。
  治它的是 **`1ca8675`「the second leg of an internal call names the first」(08-22 16:12)**,
  最后一行出现在 08-22 06:24,比修复早十小时;**08-22 12:00 之后 34 通内部呼叫,零例**。
  **二、顺手查出的这条今天还在发生,而且是根因。**
  `answered_at < started_at` 共 64 行,**拆开看是三件事**:
  ```
  INTERNAL  15 行  最大提前 6.94s  末次 08-22   ← 同一个病,同一个药(1ca8675)
  OUTBOUND  19 行  最大提前 2.64s  末次 08-23
  INBOUND   30 行  最大提前 0.032s 末次 08-25   ← 量级完全不同,仍在发生
  ```
  08-24 之后:INTERNAL 12 通 0 例、OUTBOUND 11 通 0 例、**INBOUND 6 通仍有 1 例**。
  **INBOUND 那 32 毫秒不是"记录被拆开",是两个时钟在赛跑** ——
  `answeredAt` 与 `endedAt` **一直**来自交换机盖在事件上的时间戳(`ev.OccurredAt`),
  而 `startedAt` 来自我们在 registry 建 Call 时的 `time.Now()`。
  来电是交换机先接起、事件才到我们这儿,几十毫秒的交叉是必然的。
  **三、修法:账本只用一个时钟。**
  `Registry.CreateCall` 收一个 `at time.Time`,协调器两处传 `ev.OccurredAt`。
  **收养路径本来就是对的**(`waiting.go` 用 `member.StartedAt`,建完再覆盖 `CreatedAt`)——
  一半代码早就知道该用交换机的时钟,只是正常路径没跟上;现在改成直接传进去,那行覆盖也删了。
  **影响面比"偶尔倒挂"大**:`total_sec = EndedAt - StartedAt` **一直在混用两个时钟**,
  所以账本里每一行的通话时长都带着这份偏差。
  用例 `TestACallBeginsWhenTheSwitchSaysItDid` 把交换机时钟设在两小时之外,
  `time.Now()` 不可能蒙对;反向验证改回 `time.Now()`,用例立刻红。
  **现场实测(新代码)**:`started 11:32:52.992632 / answered 11:32:53.002649` ——
  应答晚于开始 10 毫秒,顺序正确,三个时间戳同源同精度。
  **未回填历史行**:旧行的 `started_at` 已经写下,重算需要事件流而事件流不留存;
  64 行的偏差留在原地,本条即是它们的说明。
- **C58(2026-08-25,由 C57 的小账①查出)** **【已修 `764ad9e`;owner 定:留】**
  **⚠ 先更正我自己的定性**:我最初把它写成"可修的真缺口",**措辞重了**。查完之后它的实际分量是:
  **不丢 CDR 行**(`hasBotLeg` 为 false,归属没变,行照写)、**不影响下一通**、**通话本身毫发无损**
  (bot 腿在转接时早就挂了,重启杀不到它;主叫由交换机握着,坐席照样派得到)。
  **唯一的损失是那一行的 `bot_sec` / `flow_id`。** 按 owner 的判据("别影响下一通就行")
  **它够不上立案**。留着的理由只剩一条,但成立:**印就在同一条通道上,而收养代码隔两行就在读通道变量,
  读它比不读还便宜**。owner 2026-08-25 定:**留**。
  以下为查证经过。
  **先把 74 行拆开,三堆完全不同的东西:**
  ```
  24 行  没有 DID —— 打到未路由的号码,或分机间通话被判为 INBOUND。从没见过 bot。bot_sec=0 是对的
  49 行  DID 95001/95002,ANSWERED/NO_ANSWER,全部落在 2026-08-17…21 —— 开发期重启最密集那几天
   1 行  01a02dc8,2026-08-23 —— 唯一的近期个案
  ```
  **74 行的共同点是 `flow_id IS NULL`** —— 而 `flow_id` 与 `bot_sec` 出自**同一份**盖在主叫通道上的印
  (`botShare()` 读 `variable_aicc_bot_sec` / `aicc_flow_id` / `aicc_bot_summary` / `aicc_bot_reason`)。
  **01a02dc8 已查明,而且不是猜的**:`transcripts` 里有 **10 行(BOT 5 / CUSTOMER 5)**,
  08:42:16–08:42:43 —— **bot 确实和主叫聊了约 28 秒**;而日志文件名说 **08:42:46 应用重启**,
  比最后一行转写晚 3 秒。日志里那一句是 `"adopted a caller queued across a restart"`。
  即:bot 说完话把人转进队列 → **应用重启** → 新进程从交换机的成员表把这位主叫收养成一条新的 Call,
  而 bot 那一段活在已经死掉的进程里。`bot_sec=0`、`flow_id=NULL`、legs 里没有 BOT 段,
  **可通话确实先经过了 bot** —— 转写是铁证。
  **08-23 之后 22 通带 DID 的来电,只有这 1 通如此** —— 不是系统性的,是重启的代价。
  **但这一处是可以修的,这才是本条立案的理由**:`waiting.go:385-390` 的收养路径
  **已经在读通道变量**了(`aicc_call_type`、`aicc_language`),却**没有读 bot 的那几个**。
  而**那些印就在同一条通道上,重启动不了它** —— 通道是交换机的,进程死了它还在;
  何况这位主叫已经在队列里,说明转接早已完成、印早已盖下。
  **修法**:收养时按 `botShare()` 同样的口径把 bot 的那一份读回来,填进这条重建的 Call。
  **代价与风险**:多几次 `uuid_getvar`,只在收养路径上、每位被收养的主叫一次;
...
- **重启的善后:BYE 带 Reason + 两条 WARN;兜底链已现场验证** **【2026-08-25;含一处我自己的更正】**
  ...
  **【owner 2026-08-25 定调:留,而且理由是我没想到的那个 —— 抓包侧的可观测性】**
  Reason 头**不是发给交换机的,是发给抓包的**。Homer(或任何记录信令的东西)手上**只有那条 BYE** ——
  它看不见本进程的日志、账本、通道变量。**裸 BYE 在那里和"主叫挂断"长得一模一样**,
  于是"这三百通是不是被一次发布切断的"这个问题,抓包答不上来。带上 Reason,它就变成一次检索。
  这个理由**不依赖对端是否解析它**,所以上面那两条实测虽然否定了我原来的说法,却没有动摇这个头本身。
  代码注释已按实测与本裁定改写。
  ...
  **是这几个时长字段里唯一被校验过的**;其余四个从来只有一个来源,没有任何东西反驳它们。

- ~~**C57(new,2026-08-24 我自己踩进去才发现,未查 —— owner:立案,以后再查)**~~ **【原始记录,保留不改】**
...
3. C1:依赖 tier 正确性的 case 已全部跑完,**修复不再被阻塞**。但注意 **C1 的症状已潜伏** ——
   ...
   **【2026-08-25 已决:W9 整项撤销,账号全部保留】** —— owner 直裁,依据是路线而不是这批账号本身:
   **一期生产只支持 web-sip-phone Chrome 扩展,原生 SIP 话机(含硬话机)是后期要支持的。**
   这样 1002 就不是遗留物料,而是**下一期那类终端在今天的唯一样本**;
   一期还没做的支持,不该在一期先把验证它的能力删掉。这道排序闸随之作废,不再是任何事的前置。
