...
| R4 | A stable extension ID: a `key` in the manifest, or a Web Store listing. | The ID is part of every `chrome-extension://` deep link. Today an unpacked install gets a random ID per machine, which is why the SPA carries the `VITE_WEB_SIP_PHONE_ID` build override for development builds. A shipped extension should not need it. |
