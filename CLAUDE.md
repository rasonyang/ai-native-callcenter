...
**UI is optional. API is the product (owner directive 2026-08-31).** `docs/openapi.json` is not documentation of the product — it *is* the product surface; the embedded SPA is one consumer of it, no more privileged than a customer's integration. In practice: no route exists that the contract does not declare (85 = 85 today, both diffs empty, `TestEveryContractOperationIsRouted`); no capability is reachable by a session cookie that a properly-scoped API key cannot reach; and a rule written because "the panel does not need it" is a rule in the wrong place. When a screen and the contract disagree about what an operation means, the contract is right. What still violates this, and what is over-built for a single-box deployment: `docs/api-first-audit.md`.
